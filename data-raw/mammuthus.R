...
raw_data <- read.csv("data-raw/Mammuthusc14Dates.csv",
                     header = TRUE,
...
mammuthus <- data.frame(lab_code = raw_data$X14C.Lab.No.,
                     site_code = raw_data$Site.Code,
                     location = raw_data$Locality,
                     c14_age = raw_data$X14C,
                     c14_sig = raw_data$X1..Sigma)

mammuthus$f14c <- exp(-mammuthus$c14_age / 8033)
mammuthus$f14c_sig <- mammuthus$f14c * mammuthus$c14_sig / 8033

usethis::use_data(mammuthus, overwrite = TRUE)
