...
raw_data <- read.csv("data-raw/Alcesc14Dates.csv",
                     header = TRUE,
...
alces <- data.frame(lab_code = raw_data$X14C.Lab.No.,
                    site_code = raw_data$Site.Code,
                    location = raw_data$Locality,
                    c14_age = raw_data$X14C,
                    c14_sig = raw_data$X1..Sigma)

alces$f14c <- exp(-alces$c14_age / 8033)
alces$f14c_sig <- alces$f14c * alces$c14_sig / 8033

usethis::use_data(alces, overwrite = TRUE)
