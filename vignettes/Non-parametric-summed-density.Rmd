...
## Estimation of the shared underlying density $f(\theta)$
...
After running the sampler for a large number of iteration (and until we are confident of convergence) we obtain estimates for the calendar age $\theta_i$ of each sample, and an estimate for the shared calendar age density $\hat{f}(\theta)$. These latter estimates of shared calendar age density take are what are called _predictive estimates_, i.e., the provide estimates of the calendar age of a hypothetical new sample (based on the set of $N$ samples that we have observed).     
...
TODO - explain that if the underlying density is close to a mixture of uniform phases then this method will not work well (as you cannot approximate a uniform phase well with a mixture of normal distributions). Should use PPcalibrate instead
