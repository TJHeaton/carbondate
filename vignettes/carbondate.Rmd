...
- summarisation of the underlying calendar age distribution from which the samples arise (i.e., estimating $f(\theta)$) based on the observed samples), provides a potentially useful proxy for activity, population size, the frequency of past events. This approach uses the principle of _dates-as-data_ [@rick1987]

We provide two different, albeit ideologically linked, approaches to achieve this:

- `PolyaUrnBivarDirichlet()` and `WalkerBivarDirichlet()` --- A non-parametric Bayesian density estimation approach using a Dirichlet process mixture model (DPMM) implemented via two different algorithms [@heaton2021]. This is a statistically-rigorous alternative to summed probability distributions (SPDs) and kernel density estimates (KDE models).
...
Separate vignettes are provided to describe each approach. Which approach is more appropriate to apply will be situation dependent, and will be determined by the nature of the underlying shared calendar age distribution (the unknown $f(\theta)$):

- Can the set of observed samples be well approximated by a mixture of (potentially overlapping) clusters of samples? In which case `PolyaUrnBivarDirichlet()` or `WalkerBivarDirichlet()` are likely most suitable? 
- Or is it more likely to be characterised by distinct changepoints in the rate at which samples occur? In which case `PPcalibrate()` will be more appropriate     
...
The reliability of such a _dates-as-data_ approach is highly dependent upon our ability to estimate the calendar ages of the discoveries. Most archaeological/environmental dates are obtained using radiocarbon. The need for calibration of these ^14^C samples introduces considerable uncertainties in the resultant calendar ages and complicates the use of _dates-as-data_, making it more challenging to identify variations in the calendar year rates at which samples occur.
...
## Data Requirements
The (most-automated) version of the library only requires vectors:

- The ^14^C determinations $X_i$: `rc_determinations`
- The accompanying (lab-reported) measurement uncertainties $\sigma_i$: `rc_sigmas`

**Note:** These values can either be provided as ^14^C ages, or as F^14^C concentrations. 
