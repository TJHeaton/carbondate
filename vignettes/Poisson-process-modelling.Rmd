---
title: "Poisson Process Modelling and Summarisation"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Poisson Process Modelling and Summarisation}
...
This component  of the library allows you to model (and estimate) the occurrence of radiocarbon samples as a variable-rate (inhomogeneous) Poisson process. This is done using the function `PPcalibrate()`. The aim of this function is primarily to allow a user to estimate the variable (calendar year) rate at which a set of radiocarbon samples are observed, and identify changepoints in that rate. For full technical details of the models used, and explanation of the model parameters, see @heaton2025. 
...
This is a basic example which shows you how to solve a common problem. In this simulated example, we will assume that the events (samples) must lie within the calendar period from [700, 300] cal yr BP. Within this period, we will 
create 40 artificial samples with $^{14}$C determinations corresponding to the (narrower) calendar period from [550, 500] cal yr BP. Effectively, the underlying model for the occurrence of the samples is then a Poisson process with a rate: 
$$
\lambda(t) = \left\{ \begin{array}{ll}
  ...
  \end{array} \right.
$$
that has two changepoints (at 550 cal yr BP and at 500 cal yr BP). Equivalently, the model can be viewed as a uniform phase. We seek to investigate if we can reconstruct this underlying calendar age information, from just the $^{14}$C values and the initial knowledge that the samples lay between the much wider bounding interval from [700, 300] cal yr BP. 
...
that allow you to annotate the summary plots showing the posterior mean rate and the changepoint locations. Suppose for our simulated example we want to highlight the underlying [550, 500] cal yr BP interval in the plots:

```{r annotate_plot_mean_rate, out.width="100%"}
# Run plotting function and assign output to posterior_mean_plot
posterior_mean_plot <- PlotPosteriorMeanRate(PP_fit_output)
...
```
We can similarly annotate the plot showing the posterior location of the changepoints: 

```{r annotate_plot_changepoints, out.width="100%"}
...
AddTextPlot(posterior_changepoint_plot,
    ...
    col = "purple")
```
See the [independent calibration vignette](Independent-calibration.html) for more examples of ways to annotate the summary plot. 
