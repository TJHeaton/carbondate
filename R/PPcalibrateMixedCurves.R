...
#' Variable-Rate Poisson Process when the Samples Come from a Variety
#' of Environments and Multiple Calibration Curves are Needed
#'
#' @description
...
#'
#' Importantly, this function allows for different samples to be calibrated against different curves
#' For example, if some of the samples come from Marine environments (and so should be calibrated
#' against the relevant MarineCal curve with a \eqn{\Delta R} offset) while other samples are atmospheric
#' and are to be calibrated against the relevant IntCal (NH) or  SHCal (SH) calibration curve.
#'
...
#'
#' @param sample_source A vector describing the source of each \eqn{{}^{14}}C sample and determining the
#' calibration curve to use. Choices for the values in the vector are `"NH"`, `"SH"` and `"Marine"`.
#' Must be the same length as `rc_determinations`
#'
#' @param curve_year One of `"2020"`, `"2013"`, `"2009"` or `"2004"` specifying the calibration curve year
#' to use. Make sure you use the \eqn{\Delta R} appropriate to the calibration curve.
#'
#' @param delta_r,delta_r_sig A vector of the \eqn{\Delta R} offset and associated 1\eqn{\sigma} uncertainty
#' for each sample. For atmospheric samples, select `0` unless you want to model them as offset from the
#' relevant curve.
#'
...
PPcalibrateMixedCurves <- function(
  ...
  .CheckChoice(arg_check, curve_year, c("2020", "2013", "2009", "2004"))
  .CheckChoiceVector(arg_check, sample_source, c("NH", "SH", "Marine"))
  .CheckNumberVector(arg_check, delta_r, len = length(rc_determinations))
  .CheckNumberVector(arg_check, delta_r_sig, lower = 0, len = length(rc_determinations))
  .CheckAtmosphericDeltaR(arg_check, sample_source, delta_r, delta_r_sig)
  ...
  cal_curves <- .CreateCalCurveList(curve_year)
  ...
  bounds_calendar_range <- .FindBoundingCalendarRangeMultipleCurves(
    rc_determinations = rc_determinations,
    rc_sigmas = rc_sigmas,
  ...
  if (any(is.na(calendar_age_range))) {
    ...
    max_min_cal_age_curves <- max(sapply(cal_curves, function(x) min(x$calendar_age_BP)))
    min_max_cal_age_curves <- min(sapply(cal_curves, function(x) max(x$calendar_age_BP)))

    min_potential_calendar_age <- max(min_potential_calendar_age,
                                      max_min_cal_age_curves)
    max_potential_calendar_age <- min(max_potential_calendar_age,
                                      min_max_cal_age_curves)
  } else {
