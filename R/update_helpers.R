...
.UpdatePhiTau <- function(theta, mu_phi, lambda, nu1, nu2) {
  ...
  nu1new <- nu1 + nclus / 2
  nu2new <- nu2 + 0.5 * ((nclus * s) + (lambda * nclus * ((thetabar - mu_phi)^2) / (lambda + nclus)))
...
# Function which updates mu_phi via Gibbs sampling based upon current (phi, tau) values
# Arguments:
...
.UpdateMuPhi <- function(phi, tau, lambda, A, B) {
  locprec <- tau * lambda # We then observe phi_i ~ N(mu_phi, prec = lambda*tau_i)
...
.ThetaLogLikelihood <- function(z, prmean, prsig, c14obs, c14sig, mucalallyr, sigcalallyr) {
