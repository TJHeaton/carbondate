...
An R package to analyse, and summarise, multiple radiocarbon (¹⁴C)
determinations. The package provides two linked (but distinct) Bayesian
approaches that can both be used to obtain rigorous and robust
alternatives to summed probability distributions (SPDs):
...
- Variable-Rate Poisson Process Estimation (Modelling changes in the
  occurrence rate of the ¹⁴C samples over calendar time) — see “A New
  ...
  using a Poisson Process” [(Heaton, Al-assam & Bard,
  2025)](https://doi.org/10.1016/j.jas.2025.106237)
...
![](reference/figures/README-plot_density-1.png) The summary estimates
generated by the Bayesian non-parametric approach (shown in purple and
green) are called predictive densities. They aim to predict the calendar
age of a future (new) sample based upon the set of ¹⁴C samples that you
have summarised.
...
![](reference/figures/README-plot_mean_rate-1.png) This estimated sample
occurrence rate, and changes within it, can be interpreted equivalently
