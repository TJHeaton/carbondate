...
sample; and broader output (the weights, means and variances of the
...
by slice sampling (Walker 2007). We recommend use of the alternative to
this, implemented at
...
Walker, SG. 2007. “Sampling the Dirichlet Mixture Model with Slices.”
...
A list with 11 items. The first 8 items contain output of the model,
...
- `cluster_identifiers`:
  ...
  Provides the cluster allocation (an integer between 1 and `n_clust`)
  for each observation on the relevant MCMC iteration. Information on
  the state of these calendar age clusters (means, precisions, and
  weights) can be found in the other output items.
...
- `phi`:
  ...
  the means of the current calendar age clusters \\\phi_j\\.

- `tau`:

  A list of length \\n\_{\textrm{out}}\\ each entry giving a vector of
  the precisions of the current calendar age clusters \\\tau_j\\.

- `weight`:

  A list of length \\n\_{\textrm{out}}\\ each entry giving the mixing
  weights of each calendar age cluster.
...
- `input_parameters`:
  ...
  `nu1`, `nu2`, `A`, `B`, `alpha_shape`, and `alpha_rate`, and the slice
  parameters `slice_width` and `slice_multiplier`.

## See also

[PolyaUrnBivarDirichlet](https://tjheaton.github.io/carbondate/reference/PolyaUrnBivarDirichlet.md)
for our preferred MCMC method to update the Bayesian DPMM (otherwise an
identical model); and
